/*
  calcrc32() -- compute the CRC-32 of a data stream
  caladler32() -- compute the Adler-32 checksum of a data stream
 */
 ...
 */
const uint32 * get_crc_table_ake()
{
#ifdef DYNAMIC_CRC_TABLE
    if (crc_table_empty) 
        make_crc_table();
...
#define ADLDO1(buf,i)  {s1 += buf[i]; s2 += s1;}
#define ADLDO2(buf,i)  ADLDO1(buf,i); ADLDO1(buf,i+1);
#define ADLDO4(buf,i)  ADLDO2(buf,i); ADLDO2(buf,i+2);
#define ADLDO8(buf,i)  ADLDO4(buf,i); ADLDO4(buf,i+4);
#define ADLDO16(buf)   ADLDO8(buf,0); ADLDO8(buf,8);
